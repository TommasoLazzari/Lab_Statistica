...
REPORTS_PATH = BASE_DIR / "reports" / "level4_results"
...
# --- ANOMALY SCORES ---
print("Caricamento anomaly scores...")
def load_scores(filename):
    path = REPORTS_PATH / filename
    if not os.path.exists(path):
        return pd.DataFrame(columns=["node_id", "final_score", "attribute_score", "structural_score"])
    df = pd.read_csv(path)
    return df.set_index("node_id")

scores_SA  = load_scores("anomaly_scores_SA.csv")
scores_CIG = load_scores("anomaly_scores_CIG.csv")
scores_AGZ = load_scores("anomaly_scores_AGZ.csv")
scores_AG  = load_scores("anomaly_scores_AG.csv")
...
# Posizionamento Macro: attribute_score (x) e structural_score (y)
df_macro["x"] = df_macro["id"].map(
    lambda i: scores_SA.loc[i, "attribute_score"] if i in scores_SA.index else np.nan
)
df_macro["y"] = df_macro["id"].map(
    lambda i: scores_SA.loc[i, "structural_score"] if i in scores_SA.index else np.nan
)
df_macro["final_score"] = df_macro["id"].map(
    lambda i: scores_SA.loc[i, "final_score"] if i in scores_SA.index else np.nan
)
# Manteniamo solo le SA con score disponibile
df_macro = df_macro.dropna(subset=["x", "y"]).reset_index(drop=True)
...
def update_macro(search_value):
    ...
    fig.add_trace(go.Scattergl(
        ...
        customdata=np.stack((filtered_df["id"], filtered_df["count"], filtered_df["final_score"]), axis=-1).tolist(),
        hovertemplate=(
            "<b>%{text}</b><br>" +
            "ID SA: %{customdata[0]}<br>" +
            "Contratti (CIG): %{customdata[1]:,}<br>" +
            "Anomaly Score: %{customdata[2]:.4f}<br>" +
            "Attribute Score: %{x:.4f}<br>" +
            "Structural Score: %{y:.4f}<br>" +
            "<extra></extra>"
        )
    ))
    ...
    fig.update_layout(
        ...
        hovermode='closest',
        xaxis=dict(title="Attribute Score", showgrid=True, zeroline=False, type="log"),
        yaxis=dict(title="Structural Score", showgrid=True, zeroline=False, type="log"),
    )
    return fig
...
def update_cigs(clickData):
    ...
    # Posizionamento CIG: attribute_score (x) e structural_score (y)
    cig_scores = scores_CIG.reindex(c_ids)
    cx = cig_scores["attribute_score"].values
    cy = cig_scores["structural_score"].values
    c_final = cig_scores["final_score"].values
...
def update_awards(clickData):
    ...
    # --- Nodo CIG centrale ---
    cig_score_row = scores_CIG.loc[cig_id] if cig_id in scores_CIG.index else None
    cig_x = float(cig_score_row["attribute_score"]) if cig_score_row is not None else 0.0
    cig_y = float(cig_score_row["structural_score"]) if cig_score_row is not None else 0.0

    nodes_x, nodes_y, texts, colors = [cig_x], [cig_y], [f"CIG: {cig_code}"], ['#1f77b4']

    for az in agz_ids:
        agz_row = scores_AGZ.loc[az] if az in scores_AGZ.index else None
        ax = float(agz_row["attribute_score"]) if agz_row is not None else cig_x
        ay = float(agz_row["structural_score"]) if agz_row is not None else cig_y
        nodes_x.append(ax); nodes_y.append(ay)
        colors.append('#ff7f0e'); texts.append(f"Aggiudicazione {az}")
        fig.add_shape(type="line", x0=cig_x, y0=cig_y, x1=ax, y1=ay, line=dict(color="#ccc", dash="dot"))

        mask_g = edge_agz_ag[0] == az
        for gid in edge_agz_ag[1, mask_g]:
            ag_row = scores_AG.loc[int(gid)] if int(gid) in scores_AG.index else None
            gx = float(ag_row["attribute_score"]) if ag_row is not None else ax
            gy = float(ag_row["structural_score"]) if ag_row is not None else ay
            nodes_x.append(gx); nodes_y.append(gy); colors.append('#2ca02c')
            w_name = AG_id.get(int(gid), {}).get('denominazione', 'N/A')
            texts.append(f"VINCITORE: {w_name}")
            fig.add_shape(type="line", x0=ax, y0=ay, x1=gx, y1=gy, line=dict(color="#99ff99"))
